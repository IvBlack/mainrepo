	...
	class SQL {
		...
		public function Select($table, $where_key = false, $where_value = false, $fetchAll = false) {
			...
			if ($where_key AND $where_value) {
				$query = "SELECT * FROM " . $table . " WHERE " . $where_key . " = '" . $where_value . "'"; //если подставить запрос из двух таблиц "SELECT order_id, product_id, count, title, price FROM basket AS T1 INNER JOIN products AS T2 ON T1.product_id = T2.id WHERE T1.user_id = " . $_SESSION["user_id"], то слетает авторизация.
			} else {
